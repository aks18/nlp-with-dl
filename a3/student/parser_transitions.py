...
def minibatch_parse(sentences, model, batch_size):
    ...
    for i in range(len(sentences)):
        ...
        partial_parse_sentence = PartialParse(sentence)
        partial_parses.append(partial_parse_sentence)
    ...
    while len(unfinished_parses) > 0:
        ...
        transitions = model.predict(minibatch_partial_parses)
        batch_number += 1
        # print(f"len partial_parses = {len(partial_parses)}\tlen of unfinished parses = {len(unfinished_parses)}\t"
        #       f"len of transitions = {len(transitions)}\t{transitions}\t")
        for i in range(len(transitions)):
            unfinished_parses[i].parse_step(transitions[i])
            # print(f"sentence = {unfinished_parses[i].sentence}\tbuffer = {unfinished_parses[i].buffer}\t"
            #       f"stack = {unfinished_parses[i].stack}\tdepedencies = {unfinished_parses[i].dependencies}")
        temp_unfinished_parses = unfinished_parses[:]
        for i in range(len(temp_unfinished_parses)):
            if len(temp_unfinished_parses[i].stack) <= 1 and len(temp_unfinished_parses[i].buffer) <= 0:
                unfinished_parses.remove(unfinished_parses[unfinished_parses.index(temp_unfinished_parses[i])])
        # print()
    for i in range(len(partial_parses)):
        dependencies.append(partial_parses[i].dependencies)

    # print(f"len of partial parses = {len(partial_parses)}\nlen of dependencies = {len(dependencies)}\t{dependencies}")
    ### END YOUR CODE
